...
public class StreamMapTest {
	...
	public static void main(String[] args) {
		...
		warriors = persons.stream().filter(p -> p.getAge() > 20 && p.getAge() < 40)
				// the compiler will figure out which is the type of arg to construct and will
				// use each element from the list
				.map(Warrior::new); // <Class>:new is a method reference to constructor (used instead of a simple
									// lambda to call the constructor)
		warriors.forEach((w) -> System.out.println(w));

...
/**
 * Records are serialized different from ordinary classes. Its fields are
 * written in the same order as declared. When it is read, the component
 * constructor as called as normal code would need to do. It cannot customize
 * its serialization, only the method `writeReplace` can be used to create a
 * copy to be serialized.
 * 
 * The value of serialVersionUID is fixed to 0L. If we added components to the
 * record, it will contain the default value for that type. If we change the
 * existing componentes it will throw something like:
 * `java.io.InvalidClassException: CustomSerializableRecord; incompatible types
 * for field x`
 * 

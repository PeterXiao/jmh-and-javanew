	...
	public static void main(String[] args) throws InterruptedException {
		...
		try (var rs = new RecordingStream()) {
			...
			rs.onEvent("jdk.CPULoad", event -> {
				System.out.printf("\tCPU load: %f%% - %f%%%n", (event.getFloat("jvmUser") * 100),
						(event.getFloat("machineTotal") * 100));
			});
			rs.onEvent("jdk.GCHeapSummary", event -> {
				System.out.printf("\tGC - Summary - %s %f MB%n", event.getString("when"),
						(event.getFloat("heapUsed") / 1_000_000));

			});
			rs.onEvent("jdk.GarbageCollection", event -> {
				System.out.printf("\tGC - Cause %s - Durantion: %.04fms%n", event.getString("cause"),
						(event.getFloat("duration") / 1_000_000));
			});
			rs.startAsync();

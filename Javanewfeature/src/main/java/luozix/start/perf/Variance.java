/**
 * @Title: Variance.java @Package luozix.start.perf @Description: TODO(用一句话描述该文件做什么)
 *
 * @author xiaoyu xiaoyulong07@outlook.com
 * @date 2021年2月12日 下午6:01:48
 * @version V1.0
 */
package luozix.start.perf;
/**
 * @ClassName: Variance @Description: TODO(这里用一句话描述这个类的作用)
 *
 * @author xiaoyu xiaoyulong07@outlook.com
 * @date 2021年2月12日 下午6:01:48
 */
...
public class Variance {
  ...
  public static void main(String... args) {
    ...
    System.out.println(
        "Imperative version done in: "
            + measurePerf(Variance::varianceImperative, population)
            + " msecs");
    System.out.println(
        "Parallel streams version done in : "
            + measurePerf(Variance::varianceStreams, population)
            + " msecs");
    System.out.println(
        "ForkJoin version done in : "
            + measurePerf(Variance::varianceForkJoin, population)
            + " msecs");
  }
  ...
  public static double varianceStreams(double[] population) {
    ...
    double variance =
        Arrays.stream(population).parallel().map(p -> (p - average) * (p - average)).sum()
            / population.length;
    return variance;
  }
  ...
  public static double varianceForkJoin(double[] population) {
    ...
    double total =
        forkJoinPool.invoke(
            new ForkJoinCalculator(
                population,
                new SequentialCalculator() {
                  @Override
  ...
  public static class ForkJoinCalculator extends RecursiveTask<Double> {
    ...
    private ForkJoinCalculator(
        double[] numbers, int start, int end, SequentialCalculator sequentialCalculator) {
    ...
    protected Double compute() {
      ...
      ForkJoinCalculator leftTask =
          new ForkJoinCalculator(numbers, start, start + length / 2, sequentialCalculator);
      leftTask.fork();
      ForkJoinCalculator rightTask =
          new ForkJoinCalculator(numbers, start + length / 2, end, sequentialCalculator);

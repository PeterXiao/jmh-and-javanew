package luozix.start.lambdas.exams.examples.chapter7;
...
public class Testing {
	...
	public static Set<String> forEachLoggingFailure(Album album) {
		...
		album.getMusicians().filter(artist -> artist.getName().startsWith("The")).map(artist -> artist.getNationality())
	...
	public static Set<String> nationalityReportUsingPeek(Album album) {
		...
		Set<String> nationalities = album.getMusicians().filter(artist -> artist.getName().startsWith("The"))
				...
				.peek(nation -> System.out.println("Found nationality: " + nation)).collect(Collectors.<String>toSet());

...
public class SealedTypesExample {
	public static void main(String[] args) {
		...
		// we dont need to do this (nor ifs) - because we restricted the implementations
		// types (we know them)
		// else {
...
/**
 * Sealed types will help us to restrict which types can extend/implement a
 * type. Will allow us to write less verbose polymorphic code.
 *
 * Constraints: - The sealed class and its permitted subclasses must belong to
 * the same module, and, if declared in an unnamed module, the same package. -
 * Every permitted subclass must directly extend the sealed class. - Every
 * permitted subclass must choose a modifier to describe how it continues the
 * sealing initiated by its superclass: - `final` - `sealed` - `non-sealed`

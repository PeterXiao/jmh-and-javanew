 ...
 * <p>
 * 移除了 HttpResponse.BodyHandler.asString() 使用 HttpResponse.BodyHandlers.ofString() 代替功能
 *
 * @author biezhi
 * @date 2018/7/10
 */
public class Example {
    ...
    public static void downloadFile() throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(new URI("https://labs.consol.de/")).GET().build();
    ...
    public static void uploadFile() throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(new URI("http://localhost:8080/upload/"))
    ...
    public static void proxy() throws Exception {
        HttpClient client =
            HttpClient.newBuilder().proxy(ProxySelector.of(new InetSocketAddress("127.0.0.1", 1080))).build();
    ...
    public void getURIs(List<URI> uris) {
        ...
        List<HttpRequest> requests =
            uris.stream().map(HttpRequest::newBuilder).map(HttpRequest.Builder::build).collect(toList());

...
public class IntArraysTest {
	...
	public void testQuickSort1Undirect() {
		...
		for (int i = t.length - 1; i-- != 0;)
			assertTrue(t[perm[i]] <= t[perm[i + 1]]);

		t = new int[t.length];
		perm = identity(t.length);
		IntArrays.quickSortIndirect(perm, t);
		for (int i = t.length - 1; i-- != 0;)
			assertEquals(i, perm[i]);
		...
		for (int i = 10; i < 89; i++)
			assertTrue(Integer.toString(i), t[perm[i]] <= t[perm[i + 1]]);
		for (int i = 0; i < 10; i++)
			assertEquals(i, perm[i]);
	...
	public void testQuickSort2() {
		...
		for (int i = d[0].length - 1; i-- != 0;)
			assertTrue(Integer.toString(i) + ": <" + d[0][i] + ", " + d[1][i] + ">, <" + d[0][i + 1] + ", "
					+ d[1][i + 1] + ">", d[0][i] < d[0][i + 1] || d[0][i] == d[0][i + 1] && d[1][i] <= d[1][i + 1]);
	...
	public void testParallelRadixSort1() {
		...
		IntArrays.parallelRadixSort(t);
		for (int i = t.length - 1; i-- != 0;)
			assertTrue("@" + i + ": " + t[i] + " > " + t[i + 1], t[i] <= t[i + 1]);
		for (int i = 100; i-- != 10;)
			t[i] = random.nextInt();
	...
	public void testRadixSort() {
		int[][] t = { { 2, 1, 0, 4 } };
		IntArrays.radixSort(t);
		for (int i = t[0].length - 1; i-- != 0;)
			assertTrue(t[0][i] <= t[0][i + 1]);
	...
	public void testRadixSortIndirectStable() {
		...
		for (int i = 0; i < t.length; i++)
			t[i] = random.nextInt(4);
		perm = identity(t.length);
		IntArrays.radixSortIndirect(perm, t, true);
		for (int i = t.length - 1; i-- != 0;)
			if (t[perm[i]] == t[perm[i + 1]])
				assertTrue(perm[i] < perm[i + 1]);
	...
	public void testRadixSort2IndirectStable() {
		...
		IntArrays.radixSortIndirect(perm, t, u, true);
		for (int i = t.length - 1; i-- != 0;)
			assertTrue(u[perm[i]] <= u[perm[i + 1]]);
		...
		for (int i = 0; i < u.length; i++)
			t[i] = random.nextInt(4);
		for (int i = 0; i < u.length; i++)
			u[i] = random.nextInt(4);
		perm = identity(t.length);
		IntArrays.radixSortIndirect(perm, t, u, true);
		for (int i = t.length - 1; i-- != 0;)
			if (t[perm[i]] == t[perm[i + 1]] && u[perm[i]] == u[perm[i + 1]])
				assertTrue(perm[i] < perm[i + 1]);
		...
		for (int i = 10; i < 89; i++)
			assertTrue(Integer.toString(i), u[perm[i]] <= u[perm[i + 1]]);
		for (int i = 0; i < 10; i++)
			assertEquals(i, perm[i]);
		for (int i = 90; i < 100; i++)
			assertEquals(i, perm[i]);
	...
	public void testRadixSort2IndirectUnstable() {
		...
		for (int i = t.length - 1; i-- != 0;)
			assertTrue(
					i + " " + t[perm[i]] + " " + t[perm[i + 1]] + " " + u[perm[i]] + " " + u[perm[i + 1]] + "  "
							+ perm[i] + " " + perm[i + 1],
					t[perm[i]] < t[perm[i + 1]] || t[perm[i]] == t[perm[i + 1]] && u[perm[i]] <= u[perm[i + 1]]);
	}

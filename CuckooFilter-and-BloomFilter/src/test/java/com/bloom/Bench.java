...
public class Bench {
	...
	private static final List<Integer> source = IntStream.range(0, 1000).boxed().collect(toList());

	@Benchmark
	public List<Integer> parallel_collect(BenchmarkState state) {
		return source.stream().collect(ParallelCollectors.parallel(i -> i, toList(), state.executor, state.parallelism))
				.join();
	}
	...
	public List<Integer> parallel_streaming(BenchmarkState state) {
		return source.stream().collect(ParallelCollectors.parallelToStream(i -> i, state.executor, state.parallelism))
				.collect(toList());
	}
	...
	public static void main(String[] args) throws RunnerException {
		new Runner(new OptionsBuilder().include(Bench.class.getSimpleName()).warmupIterations(5)
				.measurementIterations(5).forks(1).build()).run();
	}
}

/*
 * 972ffbb @ Intel i7-4980HQ (8) @ 2.80GHz, 8u222 Benchmark (parallelism) Mode
 * Cnt Score Error Units Bench.parallel_batch_collect 1 thrpt 5 10218.766 ±
 * 131.633 ops/s Bench.parallel_batch_collect 10 thrpt 5 8096.380 ± 197.893
 * ops/s Bench.parallel_batch_collect 100 thrpt 5 2441.829 ± 207.863 ops/s
 ...
 * Bench.parallel_collect 1 thrpt 5 67.891 ± 1.357 ops/s Bench.parallel_collect
 * 10 thrpt 5 105.943 ± 7.940 ops/s Bench.parallel_collect 100 thrpt 5 597.102 ±
 * 78.423 ops/s Bench.parallel_collect 1000 thrpt 5 948.764 ± 84.439 ops/s
 ...
 * Bench.parallel_streaming 1000 thrpt 5 998.359 ± 116.077 ops/s 677be167 @
 * Intel i7-4980HQ (8) @ 2.80GHz, 8u222 Benchmark (parallelism) Mode Cnt Score
 * Error Units Bench.parallel_batch_collect 1 thrpt 5 32233.522 ± 235.439 ops/s

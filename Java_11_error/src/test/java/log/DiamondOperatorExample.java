 ...
 * <p>
 * 然而在 Java 8 中，Oracle 发现在 Diamond 操作器和匿名内部类的使用中存在一些局限性， 后来修复了这些问题并准备将其作为 Java9
 * 的一部分发布出去。
 *
 * @author LiY
 * @time 下午6:00:50
...
public class DiamondOperatorExample {
	...
	public static void main(String[] args) {
		final MyHandler<Integer> intHandler = new MyHandler<>(1) {
			@Override
			public void handle() {
		...
		final MyHandler<? extends Integer> intHandler1 = new MyHandler<>(10) {
			@Override
			void handle() {
				System.out.println("收到红包 > " + getContent() + "元");
			}
		...
		final MyHandler<?> handler = new MyHandler<>("魔法师") {
			@Override
			void handle() {

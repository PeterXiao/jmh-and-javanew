...
public class TestTimSort {
	...
	private static final Comparator<Object> NATURAL_ORDER = new Comparator<>() {
		@Override
	...
	public static void main(String[] args) {
		final int length = 100;// Integer.parseInt(args[0]);
	...
	private void generateJDKWrongElem(long X) {
		for (long newTotal; X >= 2 * minRun + 1; X = newTotal) {
			...
			if (3 * minRun + 3 <= X && X <= 4 * minRun + 1) {
				...
				// add x_1=MIN+1, x_2=MIN, x_3=MIN+2, x_4=2MIN+2, x_5=3MIN+4, x_6=X-newTotal to
				// runs
				newTotal = 8 * minRun + 9;
			}
	 ...
	 * The Y_i's are chosen to satisfy the invariant throughout execution, but the
	 * x_{i,j}'s are merged (by <code>TimSort.mergeCollapse</code>) into an X_i that
	...
	public static Integer[] JDKWorstCase(int length) {
		final TestTimSort t = new TestTimSort(length);

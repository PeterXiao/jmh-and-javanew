...
/**
 * Created by  on 7/15/2017 AD.
 */

/**
 * BloomFilter gives us either :
 * <p>
 * 1. A Definite NO 2. A Maybe YES , with some false positives
 */
...
public class BloomFilterExample {
	...
	IntStream.range(0, insertions).forEach(n -> words.put(("word" + n).toString()));
	...
	long positiveTestCount = IntStream.range(0, insertions).mapToObj(n -> ("word" + n).toString())
		.filter(element -> words.mightContain(element)).count();
	...
	long falsePositiveCount = IntStream.range(2000000, 3000000).mapToObj(n -> ("word" + n).toString())
		.filter(element -> words.mightContain(element)).count();

	System.out.println("Testing for all non-inserted elements ...");
	System.out.println(
		"Expected false positives , i.e. (fpp * expectedInsertions) approx. : " + (fpp * expectedInsertions));

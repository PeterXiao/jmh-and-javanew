...
public class StringDedupTest {
	...
	// 136M / 4.9M strings = 27.75 bytes (close to 24 bytes per String + small
	// overhead
	// Non unique, intern
	// We use 120M (+small overhead for 100 strings) until very late, but can't
	// extend ArrayList 3 times - we don't have 360M
	/*
	 * Run it with: -XX:+UseG1GC -XX:+UseStringDeduplication
	...
	fillUnique();
	/*
	 * This couple of tests compare string deduplication (first test) with string
	 * interning. Both tests should be run with the identical Xmx setting. I have
	 * tuned the constants in the program for Xmx256M, but any higher value is also
	 * good enough. The point of this tests is to show that string deduplication
	 * still leaves you with distinct String objects, each of those requiring 24
	 * bytes. Interning, on the other hand, return you existing String objects, so
	 * the only memory you spend is for the LIST object.
	 */

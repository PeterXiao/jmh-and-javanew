...
public class QuickSortDualPivotTracingEnabled {
    ...
    private void sort(final int[] input, int lowIndex, final int highIndex) {
	...
	if (pivot1 > pivot2) {
	    ...
	    System.out.println(
		    "Pivot 1 and 2 are the same. Fastwarding lowIndex until pivots are different. Low Index is now -  "
			    + lowIndex);
	}
	...
	while (i <= gt) {
		...
		System.out.println(input[i] + " is < Pivot 1 (" + pivot1 + "). Exchange " + input[i] + " with "
			+ input[lt] + " and increment both markers ");
		exchange(input, i++, lt++);
		...
		System.out.println("Pivot 2 (" + pivot2 + ") is < " + input[i] + ". Exchange " + input[i] + " with "
			+ input[gt] + " and decrement gt");
		exchange(input, i, gt--);
		...
		System.out.println(
			input[i] + " is between Pivot 1 and 2 (" + pivot1 + " & " + pivot2 + "). Just increment i");
		i++;
    ...
    public static void trace(final int lowIndex, final int highIndex, final int i, final int lt, final int gt,
	    final int length) {

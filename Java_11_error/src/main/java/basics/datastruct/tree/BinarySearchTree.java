package basics.datastruct.tree;
...
public class BinarySearchTree<Key extends Comparable<Key>, Value> {
	...
	while (currentNode != null) {
	    ...
	    final int compareResult = key.compareTo(currentNode.key);
	    if (compareResult == 0) {
    ...
    private Node put(final Node currentRoot, final Key key, final Value value) {

	if (currentRoot == null) {
	    final Node tmpNode = new Node(key, value);
	    tmpNode.size = 1;
	    return tmpNode;
	}

	final int compareResult = key.compareTo(currentRoot.key);
    ...
    private void levelOrderPrint(final Node rootNode) {
	final Deque<Node> queue = new LinkedBlockingDeque<>();
	...
	final Node returnNode = floor(root, key);
    ...
    private Node floor(final Node currentTreeRoot, final Key key) {
	...
	final int compareResult = key.compareTo(currentTreeRoot.key);
	if (compareResult == 0) {
	    ...
	    System.out.println("Result > 0 : " + currentTreeRoot + " ::: " + key);
	    final Node terminalNode = floor(currentTreeRoot.right, key);
    ...
    private int rank(final Key inputKey, final Node currentNode) {

	if (currentNode == null) {
	    return 0;
	}
	final int compareResult = inputKey.compareTo(currentNode.key);

	if (compareResult < 0) {
	    return rank(inputKey, currentNode.left);
	} else if (compareResult > 0) {
	    return 1 + currentNode.left.size + rank(inputKey, currentNode.right);
	} else {
	    return currentNode.left.size;
	}
